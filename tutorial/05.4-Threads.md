...
do/end bloklarının içine `in_thread` içindeki ilk döngümüzü sararak Sonic Pi 'a do/end blokları içindeki şeyleri do/end bloklarından sonra olan kod ile *tsss  s s s ylemiş oluyoruz. By wrapping the first loop in an `in_thread`  block we tell Sonic
