...
Az önce dediğimiz gibi, döngüler programımızın *kara delik*'leridir demiştik. Bütün kodu durdurmadan, "loop" ları (döngüleri) durdurmamızın bir yolu yoktu. İki "loop"'u aynı anda nasıl çalabiliriz? Sonic Pi 'a bir şeyi kodun geri kalanıyla aynı zamanda başlatmasını söylememiz gerekiyor. This is where threads come to the rescue.
...
do/end bloklarının içine `in_thread` içindeki ilk döngümüzü sararak Sonic Pi 'a do/end blokları içindeki şeyleri do/end bloklarından sonra olan kod ile ı zamanda çalmalarını söylemiş oluyoruz. By wrapping the first loop in an `in_thread`  block we tell Sonic
